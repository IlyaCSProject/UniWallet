...
if df.empty:
    ...
    if show_delete:
        ...
        with edit_tab:
            ...
            with ec1:
                e_date = st.date_input("Date", value=sel_row["date"].date(), key=f"e_date_{selected_id}")
            with ec2:
                e_desc = st.text_input("Description", value=sel_row["description"], key=f"e_desc_{selected_id}")
            with ec3:
                e_cat  = st.selectbox("Category", CATEGORIES,
                                      index=CATEGORIES.index(sel_row["category"])
                                      if sel_row["category"] in CATEGORIES else 0,
                                      key=f"e_cat_{selected_id}")
            ...
            with ec4:
                e_ccy = st.selectbox("Currency", CURRENCIES,
                                     ...
                                     key=f"e_ccy_{selected_id}")
            with ec5:
                e_type = st.radio(
                    "Type",
                    ["Expense (−)", "Income (+)"],
                    index=0 if sel_row["amount"] < 0 else 1,
                    horizontal=True, key=f"e_type_{selected_id}",
                )
            with ec6:
                e_amount = st.number_input(
                    "Amount", min_value=0.01, max_value=100000.0,
                    value=abs(float(sel_row["amount"])),
                    step=0.50, format="%.2f", key=f"e_amount_{selected_id}",
                )
            e_note = st.text_input("Note", value=sel_row["note"] or "", key=f"e_note_{selected_id}")
